...
class LoadSuppliersFromCsvJob implements ShouldQueue
{
    ...
    private function streamInsertValidatedRows(int $total): void
    {
        ...
        try {
            while (($row = fgetcsv($handle)) !== false) {
                ...
                $assoc = SupplierLoadService::rowAssociative($row);
                $assoc['email'] = strtolower((string) ($assoc['email'] ?? ''));

                SupplierLoadService::validateRowData($assoc, $lineNumber);

                $categoryRaw = $assoc['category'];
                $categoryValue = $categoryRaw instanceof \BackedEnum
                    ? $categoryRaw->value
                    : (string) $categoryRaw;
                ...
                if (count($buffer) >= self::CHUNK_SIZE) {
                    $this->flushSupplierChunk($buffer, $now);
     ...
     */
    private function flushSupplierChunk(array $rows, string $now): void
    {
        // PostgreSQL rejects one upsert statement when multiple rows share the same
        // conflict key ("cannot affect row a second time"). Last row wins per email.
        $rowsByEmail = [];
        foreach ($rows as $row) {
            $rowsByEmail[$row['email']] = $row;
        }
        $rows = array_values($rowsByEmail);
    ...
    private function openCsvWithValidatedHeader(): array
    {
        ...
        SupplierLoadService::stripUtf8BomFromFirstCell($headerRow);

        $expected = SupplierLoadService::expectedHeaders();
